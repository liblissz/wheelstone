...
export default function CarManager() {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setImagesFiles(Array.from(e.target.files).slice(0, 9)); // limit 9 files
  };
  ...
  const uploadImagesToCloudinary = async () => {
    ...
    for (const file of imagesFiles) {
      ...
      formData.append("upload_preset", "car_uploads"); // change if needed
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingCar) {
        ...
        setCars(
          cars.map((c) =>
            c._id === editingCar._id ? res.data.car : c
          )
        );
        ...
        {cars.map((car) => (
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-2 mt-3">
              {[car.img2, car.img3, car.img4, car.img5, car.img6, car.img7, car.img8, car.img9]
                .filter(Boolean)
                .map((img, i) => (
                  <img
                    key={i}
